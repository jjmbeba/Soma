...
const DateTimeInput = React.forwardRef<HTMLInputElement, DateTimeInputProps>((options: DateTimeInputProps, ref) => {
    ...
    const onSegmentNumberValueChange = useEventCallback(
        (num: string) => {
            ...
            if (segment.type !== 'period') {
                ...
                const updatedSegments = segments.map((s) => (s.index === segment.index ? {
                    ...segment,
                    value: newValue
                } : s));
    ...
    const onSegmentValueRemove = useEventCallback(() => {
        if (!curSegment) return;
        if (curSegment.value) {
            const updatedSegments = segments.map((s) => (s.index === curSegment.index ? {
                ...curSegment,
                value: ''
            } : s));
            setSegments(updatedSegments);
                ...
                {inputValue ? (
                    ...
                    <TooltipProvider>
                        <Tooltip>
                            ...
                            <TooltipContent>
                                <p>
                                    Please enter a valid value. The input cannot be empty and must be within the range
                                    of years 1900 to 2100.
                                </p>
                            </TooltipContent>
                        </Tooltip>
                    </TooltipProvider>
